...
def render_overview(account, market):
    ...
    with c1:
        st.markdown(
            '''<div class="module-card"><div class="module-title">灵径文化风控数据库</div><div class="module-desc">做上架前文化语义预审、颜色禁忌识别、命名与文案替代表达，避免误读与差评。</div><span class="module-chip">国家规则库</span><span class="module-chip">上架预审</span><span class="module-chip">文案替换建议</span></div>''',
            unsafe_allow_html=True,
        )
    with c2:
        st.markdown(
            '''<div class="module-card"><div class="module-title">ProfitLab 利润量化</div><div class="module-desc">做成本拆解、盈亏平衡点、情景模拟与多 SKU 对比，把“凭感觉定价”改成“有边界定价”。</div><span class="module-chip">BEP 曲线</span><span class="module-chip">情景模拟</span><span class="module-chip">多 SKU 对比</span></div>''',
            unsafe_allow_html=True,
        )
    with c3:
        st.markdown(
            '''<div class="module-card"><div class="module-title">Puente 履约清关</div><div class="module-desc">覆盖订单追踪、清关预审、报关生成、数据看板和电商接口，形成前端表达到账后交付闭环。</div><span class="module-chip">订单追踪</span><span class="module-chip">清关预审</span><span class="module-chip">系统工具</span></div>''',
            unsafe_allow_html=True,
        )
    ...
    with c:
        st.markdown('<div class="metric-tile"><div class="metric-label">当前账户</div><div class="metric-value">%s</div><div class="metric-delta good">聚焦市场：%s</div></div>' % (account, market), unsafe_allow_html=True)
    with d:
    ...
    with left:
        ...
        st.markdown('<div class="section-title">平台落地逻辑</div>', unsafe_allow_html=True)
        st.markdown(
            """
            1. **前端**：围绕低价引流、体验转译、高价品牌三层商品结构重构页面表达。  
            2. **中台**：用规则库、利润模型、看板和清关预审支撑经营判断。  
            3. **后端**：通过履约指标、清关协同和本地服务商对接，把建议转化成结果。  
            """
        )
    ...
    with right:
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown('<div class="info-callout"><b>当前版本说明</b><br>这是基于你现有两个 Streamlit 原型重构后的统一平台 Demo，重点先把三大模块合并、风格拉齐、交互补齐，再逐步接真实数据与后端接口。</div>', unsafe_allow_html=True)
